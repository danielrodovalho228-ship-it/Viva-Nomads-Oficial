...
import { portfolioMetrics, type PortfolioUnit } from "@/lib/portfolio";
import { useDashDemo, useDemoMode, DemoBadge } from "@/lib/demo/demo-mode";
import { DEMO_PORTFOLIO } from "@/lib/demo/seed";
import { formatBRL } from "@/lib/utils";

/**
 * Carteira consolidada (Plano Gestor). Modo real: começa vazia (sem imóveis
 * fictícios). Em demonstração (build sem Supabase OU modo demo do admin), a
 * fonte vira o seed em memória — nada é gravado, nada se mistura.
 */
export function CarteiraClient() {
  const demo = useDashDemo();
  const { on: adminDemo } = useDemoMode();
  const units: PortfolioUnit[] = demo ? DEMO_PORTFOLIO : [];
  ...
  return (
    <>
      <PageTitle
        title="Carteira consolidada"
        subtitle="Ocupação, receita e contratos a vencer de todos os imóveis sob sua gestão."
        action={adminDemo ? <DemoBadge /> : undefined}
      />
