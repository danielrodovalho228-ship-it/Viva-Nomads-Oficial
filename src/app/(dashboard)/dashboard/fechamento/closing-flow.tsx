...
import { DemoBadge } from "@/lib/demo/demo-mode";
...
/**
 * Fluxo do fechamento — SEMPRE a partir de um contexto real (candidatura aceita)
 * OU do exemplo de demonstração (admin). Não há mais imóvel-amostra fixo embutido:
 * imóvel, partes e valores vêm do `ctx`, e a comissão é a CONGELADA no aceite
 * (regra anti-relâmpago). Reteste QA item 2 — portão do KYC SUAVE.
 */
export function ClosingFlow({ ctx, demo }: { ctx: FechamentoContexto; demo: boolean }) {
  // Imóvel, partes e valores herdados do contexto (candidatura aceita / exemplo).
  ...
  const aceiteBR = ctx.acceptedAt
    ? ctx.acceptedAt.slice(0, 10).split("-").reverse().join("/")
    : "";
